...
class test_Amenity_BaseModel(unittest.TestCase):
    ...
    def test_instances(self):
        with patch('models.amenity'):
            ...
            self.assertCountEqual(inst_dict.keys(), expected_dict_attris)
            self.assertEqual(inst_dict['name'], 'Barbie')
            self.assertEqual(inst_dict['__class__'], 'Amenity')

            for attri, types in expectec_attris_types.items():
                with self.subTest(attri=attri, typ=types):
                    self.assertIn(attri, instance.__dict__)
                    self.assertIs(type(instance.__dict__[attri]), types)
...
class TestAmenity(unittest.TestCase):
    ...
    def test_name_attri(self):
        """Test that Amenity has attriibute name, and it's as an empty string"""
        amenity = Amenity()
    ...
    def test_to_dict_creates_dict(self):
        ...
        for attri in am.__dict__:
            if attri is not "_sa_instance_state":
                self.assertTrue(attri in new_dict)
    ...
    def test_to_dict_values(self):
        ...
        self.assertEqual(new_dict["created_at"], am.created_at.str_ftime(t_format))
        self.assertEqual(new_dict["updated_at"], am.updated_at.str_ftime(t_format))
